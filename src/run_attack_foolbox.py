...
def main():
    ...
    fmodel = PyTorchModel(ensemble_model, bounds=(0, 1))
    ...
    for batch_idx, items in enumerate(
        ...
        clean_total += int(clean_acc*args.test_batch_size)
        ...
        print(
            f"current clean accuracy:  {clean_total / ((batch_idx+1)*args.test_batch_size) * 100:.2f} %")
        print(
            f"current adv accuracy:  {adv_total / ((batch_idx+1)*args.test_batch_size) * 100:.2f} %")
