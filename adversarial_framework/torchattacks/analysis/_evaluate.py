...
__all__ = ["whitebox_test", "substitute_test"]
...
def whitebox_test(
    ...
    Input :
            ...
            adversarial_args :                   (dict)
                    attack:                          (adversarial_framework.torchattacks)
                    attack_args:                     (dict)
    ...
    if progress_bar:
        iter_test_loader = tqdm(
            iterable=test_loader, desc="Dataset Progress", unit="batch", leave=False
        )
    else:
...
def substitute_test(
    ...
    assert (
        device == model.parameters().__next__().device
    ), "Model and substitute_model should be on same device"
