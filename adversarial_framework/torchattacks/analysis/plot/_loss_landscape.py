...
def loss_landscape(
    ...
    fgm_args = dict(net=model, x=single_image, y_true=single_label, verbose=verbose)

    grad = FGM(**fgm_args)

    if norm == "inf":
        grad = grad.sign() / 255.0
    ...
    if second_direction == "adversarial":
        fgm_args = dict(
            net=model, x=single_image + first_axis, y_true=single_label, verbose=verbose
            )
    ...
    if norm == "inf":
        second_axis = second_axis.sign() / 255.0
    elif type(norm).__name__ == "int" and norm >= 1:
    ...
    second_axis -= (
        (second_axis * first_axis).sum() / (first_axis * first_axis).sum() * first_axis
        )
