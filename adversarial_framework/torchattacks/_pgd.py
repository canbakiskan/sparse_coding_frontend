...
def PGD(
    ...
    Input :
        ...
        attack_params : Attack parameters as a dictionary
            ...
            initialization: Initial value of perturbation (torch.Tensor) [optional]
    ...
    for i in restarts:
        ...
        if "initialization" in attack_params:
            perturbation = attack_params["initialization"]
        else:
        ...
        if attack_params["random_start"] or attack_params["num_restarts"] > 1:
            if attack_params["norm"] == "inf":
                ...
                additional_perturbation = 2 * torch.rand_like(x) - 1
                additional_perturbation = (
                    additional_perturbation
                    * attack_params["eps"]
                    / additional_perturbation.view(x.shape[0], -1)
                    .norm(p=attack_params["norm"], dim=-1)
                    .view(-1, 1, 1, 1)
                )
                perturbation += additional_perturbation
            perturbation = clip(
                perturbation, data_params["x_min"] -
                x, data_params["x_max"] - x
            )
            ...
            if attack_params["norm"] == "inf":
                ...
                perturbation += (
                    ...
                    * attack_params["step_size"]
                    / expected_grad.view(x.shape[0], -1)
                    .norm(p=attack_params["norm"], dim=-1)
                    .view(-1, 1, 1, 1)
                )
                perturbation = (
...
def PGD_EOT_normalized(
    ...
    for i in restarts:
        ...
        for _ in iters:
            ...
            for _ in ensemble:
                e_grad = FGM(**fgm_args)
                e_grad = e_grad / e_grad.view(x.shape[0], -1).norm(p=2, dim=-1).view(
                    -1, 1, 1, 1
                )
                expected_grad += e_grad
