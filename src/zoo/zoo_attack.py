...
class BlackBoxL2:
    def __init__(self, model, batch_size=1, confidence=CONFIDENCE,
        ...
        self.timg = torch.zeros(single_shape)
        self.tlab = torch.zeros(num_labels)
        self.const = 0.0
    ...
    def blackbox_optimizer(self, iteration):
        ...
        if self.save_ckpts:
            np.save('{}/iter{}'.format(self.save_ckpts,
                                       iteration), self.real_modifier)
...
if __name__ == "__main__":
    ...
    attack = BlackBoxL2(model, batch_size=128,
                        max_iterations=15000, confidence=0, use_log=use_log)
    ...
    print("Valid Classification:", np.argsort(
        model.model.predict(inputs[0].reshape((1,) + adv.shape)))[-1:-6:-1])
    print("Adversarial Classification:", np.argsort(
        model.model.predict(adv.reshape((1,) + adv.shape)))[-1:-6:-1])
